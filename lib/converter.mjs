...
async function convertEditable(inputPath, outputPath, options = {}) {
  ...
  try {
    ...
    // Quality audit (advisory): flag likely-duplicate/missing slides and failed fonts.
    const audit = await page.evaluate(() => {
      const djb2 = (s) => { let h = 5381; for (let i = 0; i < s.length; i++) h = (((h << 5) + h) + s.charCodeAt(i)) >>> 0; return h; };
      const slides = Array.from(document.querySelectorAll('.slide'));
      const fps = slides.map((el) => {
        const r = el.getBoundingClientRect();
        return djb2(`${Math.round(r.width)}x${Math.round(r.height)}|${el.childElementCount}|${(el.textContent || '').trim()}`);
      });
      const dups = [];
      for (let i = 0; i < fps.length - 1; i++) if (fps[i] === fps[i + 1]) dups.push([i + 1, i + 2]);
      const fontErrors = [];
      try { document.fonts.forEach((ff) => { if (ff.status === 'error') fontErrors.push(ff.family); }); } catch { /* ignore */ }
      return { dups, fontErrors };
    });

    const warnings = [];
    for (const [a, b] of audit.dups) {
      warnings.push(`Slides ${a} and ${b} are identical — possible duplicate or missing content.`);
    }
    if (audit.fontErrors.length) {
      warnings.push(`Fonts failed to load (will be substituted): ${[...new Set(audit.fontErrors)].join(', ')}`);
    }
